...
public class VirtualThreadExamples {
    ...
    static class Example0ThreadAPI {
    ...
    static class Example1ThreadAPI {
        public static void main(String[] args) throws InterruptedException {
            Thread vThread = Thread.builder().name("virtual-thread-")
                    .virtual()
                    .task(() -> System.out.println("ThreadBuilder is fun!! : "
                            + Thread.currentThread()))
                    .build();
            vThread.start();
            vThread.join();

            Thread kThread = Thread.builder().name("kernel-thread-")
                    .task(() -> System.out.println("ThreadBuilder is fun!! : "
                            + Thread.currentThread()))
                    .build();
            kThread.start();
    ...
    static class Example2CarrierThread {
        public static void main(String[] args) throws InterruptedException {
            var factory = Thread.builder().name("kernel-carrier-thread")
                    .daemon(true).factory();
            //var factory = Thread.builder().name("virtual-thread")
            //              .virtual().factory(); //throws IllegalCallerException
            ...
            Thread vThread = Thread.builder().name("virtual-thread-")
                    .virtual(executor)
                    .task(() -> System.out.println("Virtual Thread with Carrier Thread!! : "
                            + Thread.currentThread()))
                    .build();
            vThread.start();
            vThread.join();
    ...
    static class Example3KernelThreadBlocking {
        public static void main(String[] args) {
            timed(o -> {
                var factory = Thread.builder().name("kernel-thread")
                        .daemon(false).factory();

                try (var ex = Executors.newSingleThreadExecutor(factory)) {
                    IntStream.range(0, 5)
                            .forEach(i -> ex.execute(sleepTask(i)));
                }
                System.out.println("Exiting now...");
            });
        }
    ...
    static class Example3VThreadNonBlocking {
        public static void main(String[] args) {
            timed(o -> {
                ...
                var virtualThreadFactory = Thread.builder()
                        ...
                        .virtual(kernelThreadExecutor).factory();
        ...
        private static ExecutorService singleThreadKernelExecutor() {
            var kernelThreadFactory = Thread.builder()
                    .name("kernel-carrier-thread-", 0)
                    .daemon(false).factory();
    ...
    private static Runnable sleepTask(int id) {
        return () -> {
            System.out.println(format("[%s][%s] Starting time consuming task [id=%s]",
                    now(), Thread.currentThread(), id));
            try {
                Thread.sleep(Duration.ofSeconds(5));
            } catch (InterruptedException e) {
                System.out.println(format("[%s][%s] InteruptedException occurred time consuming task [id=%s]",
                        now(), Thread.currentThread(), id));
            }
            System.out.println(format("[%s][%s] Ended time consuming task [id=%s]",
                    now(), Thread.currentThread(), id));
        };
    }
}
